...
function applyingPeople(data) {
  ...
  const out = [
    {
      id: '0',
      name: [primary.firstName, primary.lastName].filter(Boolean).join(' ') || 'Primary applicant',
      dob: primary.dob,
      sex: primary.sex,
      relationship: 'Self',
  ...
  for (const m of data.householdMembers || []) {
    ...
    out.push({
      ...
      dob: m.dob,
      sex: m.sex,
      relationship: (WIZARD_RELATIONSHIPS.find((r) => r.value === m.relationship) || {}).label || '—',
      age: ageFrom(m.dob),
    });
  }
  return out;
  ...
  const IdentitySection = (
    <Panel title="Identity">
      <Stack gap={14}>
        ...
        <Field label="Race" hint="Optional — pick any that apply. This won't affect your eligibility.">
            ...
            {RACE_OPTS.map((o) => {
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    const next = on ? v.race.filter((r) => r !== o.value) : [...v.race, o.value];
        ...
        <FileUpload
          ...
          onChange={(next) => setPath('primaryApplicant', 'proofOfIdentity', next)}
        ...
        {people.map((p, idx) => {
          ...
          return (
            <div key={p.id} className="member-block">
              <MemberSectionHead name={p.name} relationship={p.relationship} age={p.age} />
              <Stack gap={12}>
                <RadioGroup
                  name={`cit-${p.id}`}
                  value={c.status}
                  ...
                  <Stack gap={12}>
                      ...
                      <Field label="Document type">
                        <Select
                          ...
                          options={[
                            ...
                            { value: 'passport_stamp', label: 'Passport with I-551 stamp' },
                    ...
                    <div className="grid-2">
                      <Field label="Date of entry to the U.S.">
                        <TextInput type="date" value={c.dateOfEntry} onChange={(x) => setCz(p.id, 'dateOfEntry', x)} />
                      </Field>
                      <Field label="Country of birth">
                        <TextInput value={c.countryOfBirth} onChange={(x) => setCz(p.id, 'countryOfBirth', x)} />
                      </Field>
                    </div>
                    <QuestionRow
                    ...
                    {c.hasSponsor === true ? (
                      <Stack gap={10}>
                        <Field label="Sponsor's name">
                          <TextInput value={c.sponsorName} onChange={(x) => setCz(p.id, 'sponsorName', x)} />
                        </Field>
                        <div className="grid-2">
                          <Field label="Sponsor's address">
                            <TextInput value={c.sponsorAddress} onChange={(x) => setCz(p.id, 'sponsorAddress', x)} />
                          </Field>
                ...
                {c.status === 'not_citizen' ? (
                  <Alert kind="info" title="You may qualify for Emergency Medicaid">
                    Emergency Medicaid covers urgent medical care regardless of immigration status. A caseworker will
                    follow up on what's available.
                  </Alert>
                ) : null}
              ...
              {idx < people.length - 1 ? <div className="divider-strong" /> : null}
            </div>
          );
        })}
      </Stack>
      ...
      <Stack gap={12}>
        ...
        {v.pregnant === true ? (
          <Fragment>
            ...
            <Alert kind="info">
              Pregnancy increases your household size for Medicaid. If you're expecting twins or more, each baby counts
...
function StepWorkRequirementsV2({ ctx }) {
  const { data, setData } = ctx;
  const people = applyingPeople(data).filter((p) => p.age !== null && p.age >= 19 && p.age < 65);
  ...
  return (
    <Stack gap={20}>
      <Alert kind="info">
        For most adults aged 19–64, State-X Medicaid requires either work, school, or an exemption. Tell us about each
        applying adult's situation.
      </Alert>
                ...
                <Stack gap={10}>
                  ...
                  <Field label="Average hours per week">
                    <TextInput
                      ...
                      onChange={(x) => set(p.id, { hoursPerWeek: Number(x) || 0 })}
                    />
                  </Field>
                  {w.hoursPerWeek > 0 ? (
                    <div className={'req-pill ' + (meetsThreshold ? 'ok' : 'warn')}>
                      <Icon name={meetsThreshold ? 'check' : 'alert'} size={14} />
                      {meetsThreshold ? (
                        <span>Meets the 80 hr/month requirement ({Math.round(monthlyHours)} hrs/mo)</span>
                      ) : (
                        <span>
                          Below the 80 hr/month requirement — needs {Math.round(80 - monthlyHours)} more hrs/mo or an
                          exemption
                        </span>
                      )}
                    </div>
              ...
              {w.employedOrInActivity === false ? (
                <Stack gap={10}>
                  ...
                  {(w.exemptions || []).includes('none') ? (
                    <Alert kind="warning">A caseworker will contact you to discuss your options.</Alert>
                  ) : null}
                </Stack>
              ) : null}
            </Stack>
      ...
      <div className="fineprint" style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
        <Icon name="info" size={12} /> Exemption rules pending state policy confirmation.
      </div>
    </Stack>
  );
}
...
function MemberSheet({ open, onClose, initial, onSave }) {
  ...
  const valid = draft.firstName && draft.lastName && draft.dob && !validateDob(draft.dob) && draft.relationship;
    ...
    <div className="sheet-overlay" onClick={onClose}>
      ...
      >
        <div className="sheet-head">
          <h3 id={titleId}>{initial ? 'Edit household member' : 'Add household member'}</h3>
          <button type="button" className="btn btn--ghost size-sm" aria-label="Close" onClick={onClose}>
          ...
          <Stack gap={14}>
                ...
                <Select
                  ...
                  onChange={(x) =>
                    setDraft({
                      ...draft,
                      sex: x,
                      ...(x !== 'female' && { pregnant: null, dueDate: '', expectedBabies: 1 }),
                    })
                  }
                  options={[
                    { value: 'male', label: 'Male' },
            ...
            <div className="grid-name">
              <Field label="First name" required>
                <TextInput value={draft.firstName} onChange={(x) => setDraft({ ...draft, firstName: x })} />
              </Field>
              <Field label="MI">
                <TextInput
              ...
              <Field label="Last name" required>
                <TextInput value={draft.lastName} onChange={(x) => setDraft({ ...draft, lastName: x })} />
              </Field>
            </div>
            <div className="grid-2">
              <Field label="Date of birth" required error={draft.dob ? validateDob(draft.dob) : undefined}>
                <TextInput type="date" value={draft.dob} onChange={(x) => setDraft({ ...draft, dob: x })} />
              </Field>
              <Field label="SSN" htmlFor="sheet-ssn" hint="Optional. If they have one.">
                <SSNInput id="sheet-ssn" value={draft.ssn} onChange={(digits) => setDraft({ ...draft, ssn: digits })} />
              </Field>
            </div>

            {age !== null ? <div className="fineprint">{age} years old</div> : null}
                  ...
                  <div className="grid-2" style={{ marginTop: 12 }}>
                    ...
                    <Field label="Number of babies">
                      <TextInput
                        ...
                        onChange={(x) => setDraft({ ...draft, expectedBabies: Math.max(1, Number(x) || 1) })}
                      />
                    </Field>
                  </div>
                ) : null}
        ...
        <div className="sheet-foot">
          ...
          <Button variant="outline" disabled={!valid} onClick={() => save(false)}>
            Save &amp; add another
          </Button>
...
function StepHouseholdMembersV2({ ctx }) {
  ...
  const setTaxFiling = (patch) => updateFormData('taxFiling', { ...taxFiling, ...patch });
  ...
  useStepSubmit(
    useCallback(async () => {
      ...
      if (!success) {
        ...
        if (errors[0]?.code) {
          console.error('[StepHouseholdMembersV2] wireHousehold failed', { code: errors[0].code });
        }
        return { ok: false, error: 'Unable to save your household. Please try again.' };
      }
      return { ok: true };
  ...
  const remove = (id) => {
    ...
    setTaxFiling({
      headOfHouseholdId: taxFiling.headOfHouseholdId === id ? '0' : taxFiling.headOfHouseholdId,
      filingJointlyWithId: taxFiling.filingJointlyWithId === id ? '' : taxFiling.filingJointlyWithId,
    ...
    return (
      <div className={'hhm-card' + (pinned ? ' pinned' : '')}>
        ...
        <div className="hhm-actions">
          {onEdit ? (
            <button type="button" className="btn btn--ghost size-sm" onClick={onEdit}>
  ...
  function Badges({ applying, disability, pregnant, filingJointly, taxDependent }) {
    return (
      <span className="hhm-badge-row" style={{ display: 'contents' }}>
        <span className={'hhm-badge ' + (applying ? 'ok' : 'muted')}>{applying ? 'Applying' : 'Not applying'}</span>
        {disability ? <span className="hhm-badge warn">Disability / LTC</span> : null}
        {pregnant ? <span className="hhm-badge accent">Pregnant</span> : null}
        {filingJointly ? <span className="hhm-badge info">Filing jointly</span> : null}
        {taxDependent ? <span className="hhm-badge neutral">Tax dependent</span> : null}
      </span>
    );
  }
  ...
  return (
    <Stack gap={20}>
      ...
      {members.length === 0 ? (
        <Alert kind="neutral">
          You live alone? That's fine — Continue. Or add anyone who shares your home, even if they aren't applying.
        </Alert>
      ) : null}

      <Panel title="Your tax household" subtitle="Medicaid uses your federal tax household to determine income limits.">
        <Stack gap={14}>
          <Field label="Will you (the primary applicant) file a federal tax return this year?">
            <RadioGroup
              ...
              onChange={(x) =>
                setTaxFiling({
                  willFile: x,
                  ...(x !== 'yes' ? { headOfHouseholdId: '0', filingJointlyWithId: '', taxDependentIds: [] } : {}),
                })
              }
              cols={3}
              options={[
            ...
            <Stack gap={12}>
              ...
              <Field label="Tax dependents">
                {members.filter((m) => m.applying).length === 0 ? (
                  <span className="fineprint">Add household members above to designate dependents.</span>
                ) : (
                  <div className="chip-group">
                    {(() => {
                      const eligible = members.filter(
                        (m) =>
                          m.applying && m.id !== taxFiling.headOfHouseholdId && m.id !== taxFiling.filingJointlyWithId,
                      );
                      if (eligible.length === 0)
                        return (
                          <span className="fineprint">
                            All household members already have tax-filing roles assigned.
                          </span>
                        );
                      return eligible.map((m) => {
                        ...
                        return (
                          <button
                            ...
                            onClick={() => {
                              const next = on
                                ? taxFiling.taxDependentIds.filter((id) => id !== m.id)
      ...
      <MemberSheet open={sheetOpen} initial={editing} onClose={closeSheet} onSave={addOrUpdate} />
    </Stack>
  );
}
...
function StepTaxDependentsV2({ ctx }) {
  ...
  const setTx = (memberId, key, val) =>
    setData((prev) => ({
      ...prev,
      taxFiling: { ...prev.taxFiling, [memberId]: { ...(prev.taxFiling[memberId] || {}), [key]: val } },
    }));
  ...
  const filers = people.filter((p) => {
    ...
    return (
      ...
      ((t.willFile === 'no' || t.willFile === 'unsure') && t.claimedByOther === true)
    );
  });
  ...
  if (filers.length === 0) {
    return (
      <Alert kind="neutral" title="No one to pick">
        Based on your tax-filing answers, there's nothing to claim or be claimed for. You can revisit this on the
        Demographics step if your situation changed.
      </Alert>
    );
  }
  ...
  return (
    <Stack gap={20}>
      <Alert kind="info">
        Your tax household determines your income limits for Medicaid. Pick the people you'll claim — or who'll claim
        you — on this year's federal return.
      </Alert>

      {filers.map((p) => {
        const t = data.taxFiling[p.id] || {};
        const others = people.filter((q) => q.id !== p.id).map((q) => ({ value: q.id, label: q.name }));
            ...
            <Stack gap={14}>
              ...
              {t.willFile === 'yes' && t.claimsDependents === true ? (
                ...
                >
                  {others.length === 0 ? (
                    <Alert kind="neutral">
                      No other household members yet. Go back and add them on the previous step.
                    </Alert>
                  ) : (
                    <Stack gap={8}>
                      {others.map((o) => (
                        <ChoiceCard
                          ...
                          onChange={(arr) => setTx(p.id, 'claimedDependents', arr)}
                          title={o.label}
                        />
                      ))}
                    </Stack>
              ...
              {(t.willFile === 'no' || t.willFile === 'unsure') && t.claimedByOther === true ? (
                <Field label={`Who will claim ${p.id === '0' ? 'you' : p.name.split(' ')[0]} as a dependent?`}>
                  {others.length === 0 ? (
                    <Alert kind="neutral">
                      No other household members yet. Go back and add them on the previous step.
                    </Alert>
