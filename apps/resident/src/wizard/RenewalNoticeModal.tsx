/**
 * RenewalNoticeModal — resident-facing, read-only "as sent" preview of the
 * pre-populated Medicaid renewal form (Form A-5170), rendered as a paper
 * document.
 *
 * This is the citizen-side counterpart to the admin app's FormPreviewModal
 * (ENG-1821). When the caseworker issues a renewal RFI, the same notice the
 * member receives in the mail is viewable here from the dashboard. Content is
 * the Diane M. Caldwell archetype (mirrors the admin `data/renewals` fixture);
 * the response deadline is taken from the case's live RFI so the "Return by"
 * date stays in sync with what the caseworker set.
 */
...
interface RenewalNoticeModalProps {
  ...
  mcNumber: string;
  /** ISO 8601 deadline from the case's rfiDetails. Falls back to a fixed display if absent. */
...
const JOB_INCOME = {
  memberName: 'Diane M. Caldwell',
  employer: 'ABC Manufacturing Co.',
  employerPhone: '(515) 555-0100',
  employerAddress: '2200 Industrial Pkwy, Capital City SX 00100',
  monthlyWages: '$1,800/mo',
  payFrequency: 'Bi-weekly',
  hoursPerWeek: '32',
} as const;

function formatDeadline(iso?: string | null): string {
  if (!iso) return 'the date on your notice';
  const d = new Date(iso);
  if (Number.isNaN(d.getTime())) return 'the date on your notice';
  return d.toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' });
}

export function RenewalNoticeModal(props: RenewalNoticeModalProps) {
  if (!props.open) return null;
  return <RenewalNoticeModalInner {...props} />;
}

function RenewalNoticeModalInner({
  onClose,
  applicantName,
  mcNumber,
  deadline,
  ...
  const dueDisplay = formatDeadline(deadline);
      ...
      >
        ...
        <div className="bg-white p-12 font-serif text-slate-900">
          ...
          <div className="mb-6 border-b pb-4">
            <p className="text-xs uppercase tracking-widest text-slate-500">State of State-X</p>
          ...
          <div className="mb-6 space-y-3 text-sm leading-relaxed">
            ...
            <p>
              <span className="font-semibold">Return your completed form by {dueDisplay}.</span> You can return it by
              mail, through your online account at the State Self-Service Portal, or by calling us. If you do not
              respond by this date, your coverage may end at the close of your current coverage period.
            </p>
          ...
          <PaperSection step="1" title="Contact information" tag="Pre-filled from your record">
            <PaperField label="Full name" value={MEMBER.name} />
            <PaperField label="Date of birth" value={MEMBER.dateOfBirth} />
            <PaperField label="Social Security Number" value={MEMBER.ssnMasked} mono />
            <PaperField label="Home address" value={MEMBER.address} />
            <PaperField label="Mailing address" value={MEMBER.mailingAddress} />
            <PaperField label="Phone" value={MEMBER.phone} />
            <PaperField label="Email" value={MEMBER.email} />
            <PaperField label="Preferred written language" value={MEMBER.preferredWrittenLanguage} />
            <PaperField label="Preferred spoken language" value={MEMBER.preferredSpokenLanguage} />
          </PaperSection>
          ...
          <PaperSection step="3" title="Income from jobs" tag="Pre-filled — confirm or update">
            ...
            <PaperField label="Person" value={JOB_INCOME.memberName} />
            <PaperField label="Employer" value={JOB_INCOME.employer} />
            <PaperField label="Employer phone" value={JOB_INCOME.employerPhone} mono />
            <PaperField label="Employer address" value={JOB_INCOME.employerAddress} />
            <PaperField label="Wages before taxes" value={JOB_INCOME.monthlyWages} />
            <PaperField label="How often paid" value={JOB_INCOME.payFrequency} />
            <PaperField label="Average hours per week" value={JOB_INCOME.hoursPerWeek} />
          ...
          <PaperSection step="4" title="Signature & certification" tag="Your signature required">
            ...
            <div className="mt-4 rounded border border-sky-200 bg-sky-50 px-3 py-2 text-xs leading-relaxed text-sky-900">
              <p className="font-semibold">Return by {dueDisplay}.</p>
              <p>
...
function PaperSection({ step, title, tag, children }: PaperSectionProps) {
  const headingId = `renewal-notice-step-${step}`;
  return (
